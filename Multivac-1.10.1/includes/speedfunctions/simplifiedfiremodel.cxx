...
#ifndef FILE_SPEEDFUNCTIONS_SIMPLIFIEDFIREMODEL_CXX

#include "simplifiedfiremodel.hxx"
...
namespace Multivac
{
   ...
   */
  template <class T>
  CSimplifiedFireModel<T>::CSimplifiedFireModel()  throw()
  {
  ...
  */
  template <class T>
  CSimplifiedFireModel<T>::
  CSimplifiedFireModel(T U_, T m_, T c_1_, T epsilon_0_, T alpha_)  throw()
  {
  ...
  */
  template <class T>
  inline void CSimplifiedFireModel<T>::Init(CMesh<T>& Mesh)
  {
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::operator() (T x, T y, T time) const
  {
    
    throw CError_Undefined
      ("T CSimplifiedFireModel<T>::operator() (T x, T y, T time) const",
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::operator() (T x, T y, T time,
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::GetMaxF1(T DxMin, T DxMax,
					     T DyMin, T DyMax,
					     T norm2) const
  {
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::GetMaxF2(T DxMin, T DxMax,
					     T DyMin, T DyMax,
					     T norm2) const
  {
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::GetDerivatives(T x, T y, T nx, T ny, T t,
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::
  ...
  */
  template <class T>
  inline T CSimplifiedFireModel<T>::Model(T costheta, T sintheta,
					  T CurrentTime) const
  {

    // Speed function:
    // If |theta| < pi / 2
    // F(theta) = epsilon_0 + c_1 * sqrt(U) * cos^m(theta)
    // else
    // F(theta) = epsilon_0 * (alpha + (1-alpha) * |sin(theta)|)
    
    if (costheta < 0)
      return ( epsilon_0 * (alpha + (1. - alpha) * abs(sintheta)) );
    else if (m!=1.5)
...
#define FILE_SPEEDFUNCTIONS_SIMPLIFIEDFIREMODEL_CXX
#endif
